...
} from "@/features/admin/calendar/services/appointment-history-logger.service";

import {
  processCompletedAppointmentLoyalty,
} from "@/features/vip/services/process-completed-appointment-loyalty.service";
...
function clean(
  ...
  if (result.length > max) {
    throw new Error(
      `Le texte ne peut pas dépasser ${max} caractères.`,
    );
  }
    ...
    if (
      ...
      await createNotification(
        systemNotification({
          ...
          actionUrl:
            `/espace-client/rendez-vous/${encodeURIComponent(
              result.appointment
                .reference,
            )}`,
        }),
